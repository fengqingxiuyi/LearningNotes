...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  created: function created() {
    ...
    // 搜索结果列表
    self.navFocus = true;
    var decodeQValue = weex.config.q;
    if (decodeQValue && decodeQValue !== '') {
      self.navFocus = false;
      if (decodeQValue.indexOf('cateName') !== -1 && decodeQValue.indexOf('brandZhName') === -1) {
        self.cateNameValue = decodeQValue.split(':')[1];
      }
      if (decodeQValue.indexOf('cateName') === -1 && decodeQValue.indexOf('brandZhName') !== -1) {
        self.brandZhNameValue = decodeQValue.split(':')[1];
      }
      if (decodeQValue.indexOf('cateName') !== -1 && decodeQValue.indexOf('brandZhName') !== -1) {
        if (decodeQValue.split('=')[1].split(',')[0].indexOf('cateName') === -1) {
          self.brandZhNameValue = decodeQValue.split('=')[1].split(',')[1].split(':')[1];
        } else {
          self.cateNameValue = decodeQValue.split('=')[1].split(',')[0].split(':')[1];
        }
      }
      self.qValue = decodeURIComponent(decodeQValue);
    }
    var decodeKeyWordValue = weex.config.keyword;
    if (decodeKeyWordValue && decodeKeyWordValue !== '') {
      self.keywordValue = decodeURIComponent(decodeKeyWordValue);
    }
  ...
  methods: {
    ...
    search: function search() {
      ...
      // TAB样式初始化
      if (self.lastKewordValue !== self.keywordValue) {
      ...
      self.fetch({
        ...
        self.lastKewordValue = self.keywordValue;
