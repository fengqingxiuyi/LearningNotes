...
__vue_options__.__file = "/Users/qingfeng/shop-weex-m/src/pages/search-result-weex/search-result-weex.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-9a7b86d8"
...
"use strict";
Object.defineProperty(__webpack_exports__, "__esModule", { value: true });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__search_result_weex_vue__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__search_result_weex_vue___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0__search_result_weex_vue__);


__WEBPACK_IMPORTED_MODULE_0__search_result_weex_vue___default.a.el = '#weex';
/* harmony default export */ __webpack_exports__["default"] = (new Vue(__WEBPACK_IMPORTED_MODULE_0__search_result_weex_vue___default.a));
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  data: function data() {
    return {
      ...
      isAndroid: weex.config.env.platform.toLowerCase() === 'android',
      isShopOwner: false,
      // public-接口
      lastKeywordValue: '',
      keywordValue: '',
      ...
      // TAB
      tabHeight: 0,
      tabArray: [{ id: 0, name: '综合', color: 'press', up: 'none', down: 'none', clickFirst: false, show: true }, { id: 1, name: '销量', color: 'normal', up: 'normal', down: 'normal', clickFirst: false, show: true }, { id: 2, name: '价格', color: 'normal', up: 'normal', down: 'normal', clickFirst: false, show: true }, { id: 3, name: '收益', color: 'normal', up: 'normal', down: 'normal', clickFirst: false, show: false }, { id: 4, name: '品牌', color: 'normal', up: 'none', down: 'normal', clickFirst: false, show: true }],
      tabItemLastClickPos: 0,
      tabBrandIsGrey: false,
      // 搜索结果列表
      resultListHeight: 0,
      resultList: [],
      resultPageIndex: 1,
      ...
      resultItemClicked: 0, // 0: 初始值，1: 点击了item的值，2: 执行了onFocus的值
      // 搜索建议列表
      sugList: [],
      sugListHeight: 0,
      // 品牌结果列表
  ...
  created: function created() {
    ...
    // 分享
    if (self.isWeb) {
      shopBase.shareInfo(self.getShareData());
    }
    // 搜索结果列表
    if (!self.isWeb) {
      self.navFocus = true;
      self.navFocusGet = true;
    }
    var localQ = decodeURIComponent(weex.config.q);
    if (localQ !== 'undefined' && localQ !== '') {
      self.navFocus = false;
      self.navFocusGet = false;
      if (localQ.indexOf('cateName') !== -1 && localQ.indexOf('brandZhName') === -1) {
        self.cateNameValue = localQ.split(':')[1];
      }
      if (localQ.indexOf('cateName') === -1 && localQ.indexOf('brandZhName') !== -1) {
        self.brandZhNameValue = localQ.split(':')[1];
      }
      if (localQ.indexOf('cateName') !== -1 && localQ.indexOf('brandZhName') !== -1) {
        if (localQ.split('=')[1].split(',')[0].indexOf('cateName') === -1) {
          self.brandZhNameValue = localQ.split('=')[1].split(',')[1].split(':')[1];
        } else {
          self.cateNameValue = localQ.split('=')[1].split(',')[0].split(':')[1];
        }
      }
      self.qValue = localQ;
    }
    var localKeyword = decodeURIComponent(weex.config.keyword);
    if (localKeyword !== 'undefined' && localKeyword !== '') {
      self.navFocus = false;
      self.navFocusGet = false;
      self.keywordValue = localKeyword;
    }
    self.resultPageIndex = 1;
    if (weex.config.shopId && weex.config.shopId !== '') {
      self.shopId = +weex.config.shopId;
    }
    if (weex.config.type && weex.config.type !== '' && weex.config.shopProductId !== 'null') {
      self.selectType = +weex.config.type;
    }
    if (weex.config.shopProductId && weex.config.shopProductId !== '' && weex.config.shopProductId !== 'null') {
      self.shopProductId = +weex.config.shopProductId;
    }
    if (self.keywordValue !== '' || self.qValue !== '') {
      // 自动展示带过来的keyword
      self.navInputContent = self.keywordValue;
      self.search();
    }
    // 搜索历史列表
  ...
  methods: {
    ...
    search: function search() {
      ...
      // 样式初始化
      if (self.lastKeywordValue !== self.keywordValue) {
        self.tabArray[self.tabItemLastClickPos].color = 'normal';
        self.tabArray[self.tabItemLastClickPos].up = 'normal';
        self.tabArray[self.tabItemLastClickPos].down = 'normal';
        self.tabArray[self.tabItemLastClickPos].clickFirst = false;
        if (self.tabItemLastClickPos === 0) {
          self.tabArray[self.tabItemLastClickPos].up = 'none';
          self.tabArray[self.tabItemLastClickPos].down = 'none';
        }
        if (self.tabItemLastClickPos === 4) {
          self.tabArray[self.tabItemLastClickPos].up = 'none';
        }
        self.tabItemLastClickPos = 0;
        self.tabArray[0].color = 'press';
      }
      // 保存搜索历史
      storage.getItem('history', function (event) {
      ...
      self.fetch({
        ...
        self.lastKeywordValue = self.keywordValue;
        if (response.isSuccess) {
          if (response.data) {
            if (response.data.shopId) {
              self.shopId = response.data.shopId;
            }
            if (response.data.isShopOwner) {
              self.isShopOwner = response.data.isShopOwner;
            }
            // TAB
            if (!self.isShopOwner) {
              self.tabArray[3].show = false;
            } else {
              self.tabArray[3].show = true;
            }
            ...
            if (self.resultPageIndex === 1) {
              if (response.data.brandNames && response.data.brandNames.length > 0) {
        ...
        // 品牌是否是置灰状态
        if (self.brandList.length > 1 && self.tabBrandIsGrey) {
          self.tabBrandIsGrey = false;
          self.tabArray[4].color = 'normal';
          self.tabArray[4].down = 'normal';
        }
      });
    },
    ...
    onFocus: function onFocus() {
      ...
      if (self.isAndroid && self.resultItemClicked === 1) {
        self.resultItemClicked = 2;
    ...
    navShare: function navShare() {
      var self = this;
      shopBase.share(JSON.stringify(self.getShareData()));
    },
    ...
    tabItemClick: function tabItemClick(tab, tabArray) {
      ...
      if (tab.id === 0) {
        ...
        self.tabItemArrowChange(tab, 'salesVolume', false);
      } else if (tab.id === 2) {
        // 价格
        shopBase.userTrack({ key: 'search_click_price', params: {} });
        self.tabItemArrowChange(tab, 'price', true);
      } else if (tab.id === 3) {
        // 收益
        shopBase.userTrack({ key: 'search_click_earn', params: {} });
        self.tabItemArrowChange(tab, 'commission', false);
      } else if (tab.id === 4) {
        // 品牌
    ...
    tabItemArrowChange: function tabItemArrowChange(tab, sort, isPrice) {
      var self = this;
      if (tab.clickFirst) {
        // 下箭头变色
    ...
    resultItemClick: function resultItemClick(result) {
      var self = this;
      if (self.isAndroid && self.resultItemClicked === 0) {
        self.resultItemClicked = 1;
      }
      shopBase.userTrack({ key: 'search_to_detail', params: {} });
      shopBase.openUrl('/shop/sku/' + result.id + '.html');
    },
    resultItemRightBottomBagClick: function resultItemRightBottomBagClick(result, e) {
      var self = this;
      if (self.isGiveaway) {
        shopModal.toast({ message: '赠品暂不支持加入购物车！' });
      }
      var queryData = {
        itemId: result.id,
        quantity: 1,
    ...
    resultItemRightBottomShelveClick: function resultItemRightBottomShelveClick(result, e) {
      var self = this;
      if (result.isSelected) {
        ...
        var _data = {
          ...
          shopProductId: self.shopProductId
        };
        self.fetch({
          method: 'GET',
          ...
          if (response.isSuccess) {
            ...
            shopModal.toast({ message: '上架成功' });
            if (self.selectType !== 3) {
              self.navBack();
            }
            if (self.selectType === 3 && self.shopProductId > 0) {
              self.navBack();
            }
          } else {
            shopModal.toast({ message: response.msg });
    ...
    resultItemRightBottomShareClick: function resultItemRightBottomShareClick(result) {
      ...
      var data = {
        ...
        specialPrice: result.isSpecialSell ? result.saleInfo.price : 0,
        specialStartTime: result.isSpecialSell ? result.saleInfo.startTime : 0
      };
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [(_vm.isWeb) ? _c('spon-header', {
    ...
    return _c('cell', {
    ...
    }, [_vm._v("特卖")]) : _vm._e(), _c('text', {
      staticClass: ["result-item-right-title-text"]
    }, [_vm._v(_vm._s(_vm._f("addSpace")('[' + result.brandZhName + '] ' + result.itemName, result.isSpecialSell)))])], 1), _c('div', {
    ...
    }, [_vm._v(_vm._s(parseFloat(result.price).toFixed(2)))]), (!_vm.isShopOwner) ? _c('text', {
      staticClass: ["result-item-right-bottom-original-price-text"]
    }, [_vm._v("￥" + _vm._s(parseFloat(result.originalPrice).toFixed(2)))]) : _vm._e()]), (_vm.isShopOwner) ? _c('yoga-tag', {
    ...
    }, [(_vm.isShopOwner) ? _c('div', {
      staticClass: ["result-item-right-bottom-shelve"],
      on: {
        "click": function($event) {
          _vm.resultItemRightBottomShelveClick(result, $event)
        }
      }
      ...
      on: {
        "click": function($event) {
          _vm.resultItemRightBottomBagClick(result, $event)
        }
      }
    ...
    })]), (_vm.isShopOwner && !_vm.isWeb) ? _c('div', {
...
/***/ })
/******/ ]);
//# sourceMappingURL=search-result-weex.weex.min.js.map
